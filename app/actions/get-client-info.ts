...
export async function get_client_info({
	document,
}: get_client_info_req): Promise<action_response> {
	try {
		const { data: body } = await axios.get(
			`https://api.cpf-brasil.org/cpf/${document}`,
			{
				headers: {
		...
		return {
			...
			data: {
				...
				message: "Avise nosso suporte para resolvermos isso",
			},
		}
	}
}
