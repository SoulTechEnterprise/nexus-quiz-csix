...
export async function send_loan({ id_simulation, client }: send_loan_req): Promise<send_loan_res> {
    const { access_token } = await get_access_token()

    const { data } = await axios.post("https://marketplace-proposal-service-api-p.c6bank.info/marketplace/worker-payroll-loan-offers/include", {
